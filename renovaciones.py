...
print("2")
label1 = tk.Label(root, text='Renovaciones', bg = 'lightsteelblue2')
...
def getExcel ():
    ...
    ws = wb.get_sheet_by_name('Renovaciones')
    my_list = []
    list_name = []
    ...
    # create csv file
    with open (r'lista_radicados_y_llaves_Renovaciones.csv', 'w', newline='') as write_file:
        write=csv.writer(write_file)
        write.writerows(filtered_list)

    aList=[]
    print("START ROW")
    with open('lista_radicados_y_llaves_Renovaciones.csv', 'r') as f:
...
def automaticQueries ():
    ...
    # EP worksheet
    with open('lista_radicados_y_llaves_Renovaciones.csv', newline='') as lista:
        reader = csv.reader(lista)
    ...
    filename = 'estado_Renovaciones.csv'
...
def convertToJSON ():
    ...
    wb.save('estado_Renovaciones_Excel.xlsx')

    messagebox.showinfo("Estado de la conversion", "Espere mientras se realiza la conversion") 

    wb = openpyxl.load_workbook('estado_Renovaciones_Excel.xlsx')
    ws = wb.active
    ...
    os.remove('estado_Renovaciones.csv')
    os.remove('lista_radicados_y_llaves_Renovaciones.csv')
